---
description: |
  Knowledge about premium credit card benefits, statement credits, and maximization strategies.
  Use when discussing credit card perks, travel credits, dining credits, or lounge access.
---

# Credit Card Benefits Knowledge

Expert knowledge for maximizing premium credit card benefits across American Express Platinum, Capital One Venture X, Chase Sapphire Reserve, Bank of America Alaska Airlines Atmos, and Delta SkyMiles Reserve cards.

## Key Deadlines (2026)

| Date | Deadline | Card |
|------|----------|------|
| Jan 31 | Select airline for $200 fee credit | Amex Platinum |
| Feb 1 | Capital One lounge guest fees begin | Venture X |
| Feb 1 | Medallion Year resets (Sky Club visits) | Delta Reserve |
| Mar 31 | Q1 credits expire (Resy, Lululemon) | Amex Platinum |
| Jun 25 | **Uber One enrollment deadline (PROMO)** | Delta Reserve |
| Jun 30 | H1 credits expire | Multiple cards |
| Sep 30 | Q3 credits expire (Resy, Lululemon) | Amex Platinum |
| Dec 31 | Calendar year credits expire | Multiple cards |
| Dec 31 | 2026 Select Hotels credit expires (does NOT continue to 2027) | CSR |

## Quick Reference: Total Annual Credit Values

| Card | Annual Fee | Total Credits | Net Value |
|------|------------|---------------|-----------|
| Amex Platinum | $895 | ~$3,700+ | +$2,800+ |
| Capital One Venture X | $395 | ~$400 | +$5 |
| Chase Sapphire Reserve | $795 | ~$1,950+ | +$1,155+ |
| Alaska Atmos Summit | $395 | ~$700+ | +$305+ |
| Delta Reserve | $650 | ~$560+ | +varies |

*Note: Delta Reserve Uber One is promotional (not standing benefit). Alaska lounge pass values are subjective.*

## Reset Type Reference

### Calendar Year (January 1)
Most credits that reset on a fixed date regardless of when you opened the card.

**Amex Platinum:**
- Hotel Credit ($300 x 2 semi-annual)
- Resy ($100 x 4 quarterly)
- Lululemon ($75 x 4 quarterly)
- Airline Fee ($200 annual - must select airline by Jan 31)
- Saks ($50 x 2 semi-annual)
- Oura Ring ($200 annual) - NEW for 2026

**Chase Sapphire Reserve:**
- The Edit Hotel ($250 x 2, flexible timing in 2026)
- Exclusive Tables ($150 x 2)
- Select Hotels 2026 ($250 one-time - does NOT continue to 2027)
- StubHub/viagogo ($150 x 2 semi-annual)
- DoorDash Restaurant ($5/mo) + Non-Restaurant ($20/mo)

### Account Anniversary
Resets on your card open date each year.

**Capital One Venture X:**
- Travel Credit ($300)
- Anniversary Miles (10,000)

**Alaska Atmos:**
- Lounge Passes (8)
- Status Points (10,000)
- Companion Fare (after $6K spend)

**Delta Reserve:**
- Delta Stays ($200)
- Companion Certificate

### Monthly
Must be used each month or forfeited.

**Amex Platinum:**
- Digital Entertainment ($25/mo)
- Uber Cash ($15/mo + $20 Dec)
- Uber One ($9.99/mo)
- Equinox ($25/mo)

**Chase Sapphire Reserve:**
- Peloton ($10/mo)
- Lyft ($10/mo)
- DoorDash Restaurant ($5/mo)
- DoorDash Non-Restaurant ($10 x 2/mo = $20/mo)

**Delta Reserve:**
- Resy ($20/mo)
- Rideshare ($10/mo)
- Uber One ($9.99/mo) ⚠️ *PROMO - must enroll by June 25, 2026*

## Strategic Stacking Opportunities

### Chase 2026 Hotel Credit Stack
In 2026 only, you MAY be able to combine credits on one hotel stay:
1. $250 from The Edit credit
2. $250 from Select Hotels credit (IHG, Montage, Pendry, Omni, Virgin, Minor, Pan Pacific)
3. $300 from annual travel credit

**Potential total: Up to $800** (stacking is possible but not guaranteed)

**Requirements:**
- Book prepaid through Chase Travel
- 2-night minimum
- Hotel must be in both The Edit AND one of the select chains
- Some properties MAY qualify for both credits; verify eligibility before booking

**Note:** The 2026 Select Hotels credit does NOT continue into 2027.

### Amex Platinum Maximum Value Month
December is the best month for Amex Platinum:
- Regular $15 Uber Cash
- Bonus $20 December Uber Cash
- End of year for Q4 credits (Resy, Lululemon)
- Use before year-end airline credits

## Enrollment Requirements

Some benefits require active enrollment:

**Amex Platinum:**
- Select your airline for the $200 fee credit (must select by Jan 31)

**Delta Reserve:**
- Resy Credit - enroll in Amex account
- Rideshare Credit - enroll in Amex account
- Uber One Credit - ⚠️ **LIMITED-TIME PROMO** - must enroll by June 25, 2026! (Not a standing benefit)

## Lounge Access Quick Guide

### Unlimited Access
- **Amex Platinum:** Centurion Lounges, Priority Pass Select
- **Venture X:** Capital One Lounges, Priority Pass (through Jan 2026)
- **Chase Sapphire Reserve:** Priority Pass, Sapphire Lounges

### Limited Visits
- **Amex Platinum:** 10 Delta Sky Club visits (when flying Delta)
- **Delta Reserve:** 15 Sky Club visits per Medallion Year (resets Feb 1; unlimited at $75K spend) + 4 guest passes/year
- **Alaska Summit:** 8 Alaska Lounge passes per year (2/quarter, use-it-or-lose-it; ~$130/quarter subjective value)

### 2026 Lounge Changes
**Capital One Venture X (Feb 1, 2026):**
- Authorized users need $125/year for lounge access
- Guests cost $45 (adults) or $25 (children 2-17)
- Free guest access restored at $75K annual spend

## Multi-Year Benefits

### Global Entry/TSA PreCheck
All five cards offer this credit, but you can only use one at a time:
- Global Entry: $120 every 4 years (includes TSA PreCheck)
- TSA PreCheck: $85 every 4.5 years

**Strategy:** Use one card's credit, then use another card's credit when it renews 4+ years later.

## Hidden/Overlooked Benefits

### Amex Platinum
- Marriott Bonvoy Gold status (free)
- Hilton Honors Gold status (free)
- Saks credit can be used on gift cards
- Airline credit can cover seat selection, bags, lounge day passes

### Venture X
- Hertz President's Circle status
- Primary car rental insurance (domestic)

### Chase Sapphire Reserve
- DashPass subscription included
- Lyft Pink membership
- Points worth 1.5x when redeemed through Chase Travel

### Delta Reserve
- 20% back on in-flight purchases
- TakeOff 15: 15% off award flights booked on delta.com

## Common Mistakes to Avoid

1. **Forgetting monthly credits** - Set calendar reminders for Uber Cash, entertainment, etc.
2. **Not selecting airline** - Amex airline credit won't work until you select an airline
3. **Missing enrollment** - Delta Reserve credits require enrollment
4. **Booking direct vs portal** - Some credits only work through specific portals (Chase Travel, Capital One Travel)
5. **Not using December Uber bonus** - Extra $20 for Amex Platinum in December
6. **Ignoring companion certificates** - Can save $200-500 on flights

## YNAB Integration Patterns

When analyzing YNAB transactions for credit card credits:

### Merchants to Look For

| Credit | Merchant Patterns |
|--------|-------------------|
| Uber | UBER, UBER EATS, UBEREATS |
| Resy restaurants | Check Resy app for booked restaurants |
| Lululemon | LULULEMON |
| Saks | SAKS, SAKSFIFTHAVENUE |
| Digital entertainment | DISNEY, HULU, ESPN, PEACOCK, PARAMOUNT, NYT, WSJ, YOUTUBE |
| Airlines | Specific airline names |
| Delta Stays | DELTA.COM |
| Chase Travel | CHASE TRAVEL |
| Capital One Travel | CAPITAL ONE TRAVEL |

### Statement Credit Identification
Look for:
- Negative transactions (credits) from AMEX, CHASE, CAPITAL ONE
- Transactions labeled "CREDIT", "STATEMENT CREDIT", "REWARD"
- Match timing to purchase date (usually 1-5 days after)
