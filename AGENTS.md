...
<!-- BEGIN CLAUDE MEMORY IMPORT: -Users-omarshahine-GitHub-Agent-Plugins -->
## Imported Claude Project Memory

Durable memory promoted from `~/.claude/projects/-Users-omarshahine-GitHub-Agent-Plugins/memory` during the AGENTS.md migration.
...
### Template Injection for Sub-Agents
When sub-agents need to produce output that includes complex HTML/CSS/JS templates:
- **Never let the agent write HTML directly** — smaller models (haiku) will rewrite from scratch
- **Separate data from presentation**: agent writes JSON data file, then a mechanical script injects it into the template
- Use `// BEGIN_MARKER` / `// END_MARKER` comments for reliable string replacement
- Use Python string splicing (not `re.sub`) to avoid unicode escape issues in replacement strings
- See `batch-html-generator.md` for the canonical example

### Version Bumping
- Always bump BOTH `plugin.json` AND `marketplace.json`
- Use **minor bumps** (1.x.0) for new features, agent architecture changes
- Use **patch bumps** (1.0.x) for bug fixes, config changes
- Current versions tracked in marketplace.json

### Bash Permission Pitfalls
- **Never write multi-line Bash scripts** — the permission system captures the first line as the pattern, producing junk rules like `Bash(# Check if...)` or `Bash(__NEW_LINE_...)` that never match again
- **Never start Bash commands with `#` comments** — the comment becomes the prefix match, which is useless
- **Use `&&` chaining** for sequential commands, not newlines
- **Use Read/Write/Edit tools** instead of `cp`/`cat` for file operations on `~/.claude/` paths — the dedicated tools have proper path-based permissions that work; Bash `cp` triggers a separate path-access check
- **For syncing settings.json**: Use `Read` + `Write` tools, NOT `cp` via Bash
- The `.claude/settings.local.json` "always allow" option generates hyper-specific rules from command text — periodically audit and clean junk entries

### PR Workflow
- After creating a PR, **immediately run `/git-pr-bugbot`** if the repo has a `.cursor/BUGBOT.md` file — don't wait for the user to notice Bugbot comments
- This avoids an extra round-trip (create PR → user sees Bugbot alert → user asks to fix → fix)
- **This repo HAS Bugbot** at `.cursor/BUGBOT.md` — the `/git-pr` skill's step 7 should trigger. Always verify Bugbot polling runs before merging, even if the check-runs API returns empty (Bugbot runs on PR commits, not squash-merge commits)
- **Never merge before Bugbot completes** — even if the conclusion is likely clean, wait for explicit confirmation

### Public/Private Fork Architecture (v2.0.0)
- **Public upstream**: `~/GitHub/Agent-Plugins` — PRs only, NOT registered as local marketplace
- **Private fork**: `~/GitHub/Agent-Plugins-Omar` — **ALL plugins install from here**
- **Remotes in private fork**: `origin` = private, `upstream` = public
- **Single marketplace**: `omarshahine-agent-plugins-private` (the only registered agent-plugins marketplace)
- **Data symlink**: `~/.claude/data/` → `~/GitHub/Agent-Plugins-Omar/data/`
- **Consolidated data**: All plugin data in `~/.claude/data/chief-of-staff/` (no separate `chief-of-staff-private/data/`)
- **Claude repo**: Global configs only (CLAUDE.md, settings.json, commands, mcp configs) — NO plugins or data
- Private plugins migrated from Claude repo: `chief-of-staff-private`, `federal-tax`, `porsche-connect`
- Public repo has PII CI guard: `.github/workflows/pii-check.yml` + `.pii-patterns.yaml`
- Public repo history was reset (orphan commit) — no PII in git history
- **Upstream PR workflow**: Detailed in `Agent-Plugins-Omar/CLAUDE.md` — branch from `upstream/main`, scrub PII, push to upstream, CI checks

### Common Issues
- Agent `model: haiku` may not follow complex multi-step instructions reliably — consider `sonnet` for agents that need to handle templates, complex data structures, or multi-file operations
- Command frontmatter: never include `name:` field (breaks autocomplete)
- Plugin cache may be stale — clear `~/.claude/plugins/cache/` if changes aren't reflected

<!-- END CLAUDE MEMORY IMPORT: -Users-omarshahine-GitHub-Agent-Plugins -->
